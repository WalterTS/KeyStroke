...
public class Cluster {
	...
	public DoubleArray getmean() {
            //System.out.println("Mean Get: "+mean.data);
		return mean;
	}
	 ...
	 */
        ...
        public void guardaCentroide(DoubleArray mean, String nombre) throws IOException {
		BufferedWriter writer = new BufferedWriter(new FileWriter("Centroides/"+nombre));		
                String centroide= mean.toString();
		writer.write(centroide);		
		writer.close();
               
	}
        
        
        public void centroide(String nombre) throws IOException{
        for(int i=0; i < sum.data.length; i++){
			mean.data[i] = sum.data[i] / vectors.size();
                        //answer.data[i]= sum.data[i] / vectors.size();
                        
		}
        System.out.println("OJO centroide: "+ mean);
        guardaCentroide(mean,nombre);
        }
